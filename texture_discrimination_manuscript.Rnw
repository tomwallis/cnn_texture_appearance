...
<<plot_appearance_fun, echo=FALSE>>=
plot_theme <- function(plt){
...
<<stat_printout_fun, echo=FALSE>>=

compute_CIs <- function(x){
  # compute CIs and p < 0 for vector of samples.
  estimates <- quantile(x, probs = c(0.025, 0.5, 0.975))
  
  # use ecdf to compute the probability that the coefficient is less than zero:
  fn <- ecdf(x)
  fn(0)
...
print_CIs <- function(x, fit=NA, hypothesis=NA, ef=NA){
  # x: a vector of posterior samples. median and 95% CIs returned.
  # fit: the fit object.
  # hypothesis: a string hypothesis testing a point null, passed 
  # to brms's hypothesis function to compute the Savage-Dickey 
  # bayes factor.
  # ef: a vector of posterior samples of the effect size. 
  
  # function to print credible intervals and p > 0.
  out <- compute_CIs(x)
  
  # formatting of P < 0:
  if (round(out$p_less_zero, 3) == 1){
    print_less_zero <- paste0(", $\\mathrm{p}(\\beta < 0) > $ ", 0.999)
  } else if (round(out$p_less_zero, 3) == 0){
    print_less_zero <- paste0(", $\\mathrm{p}(\\beta < 0) < $ ", 0.001)
  } else {
    print_less_zero <- paste0(", $\\mathrm{p}(\\beta < 0) = $ ",
                              round(out$p_less_zero, 3))
  }
  
  
  # hyp <- brms::hypothesis(fit, hypothesis)
  # # formatting of BF:
  # if (round(hyp$hypothesis$Evid.Ratio, 2) < 0.01){
  #   print_bf <- paste0(", $\\mathrm{BF_{Null}} < $ ", 0.01)
  # } else {
  #   print_bf <- paste0(", $\\mathrm{BF_{Null}} = $ ",
  #                      round(hyp$hypothesis$Evid.Ratio, 2))
  # }  
  
  # effect size computation:
  # x assumed to be mean difference, var assumed to be summed relevant variance.
  # see westfall et al 2014.
  if (!is.na(ef)){
    EFs <- compute_CIs(ef)
    
    printout <- paste0("$\\beta = $ ", 
                       round(out$estimates[2], 2),
                       ", 95\\% CI = [",
                       round(out$estimates[1], 2),
                       ", ",
                       round(out$estimates[3], 2),
                       "]",
                       print_less_zero, 
                       # print_bf, 
                       ", $ d = $",
                       round(EFs$estimates[2], 2),
                       ", 95\\% CI = [",
                       round(EFs$estimates[1], 2),
                       ", ",
                       round(EFs$estimates[3], 2),
                       "]")
  } else {
    printout <- paste0("$\\beta = $ ",  
                       round(out$estimates[2], 2),
...
<<load_experiment_1_data, echo=FALSE, cache=TRUE, results='hide'>>=

if (isTRUE(getOption('knitr.in.progress'))){
  # knitting manuscript:
  top_dir <- setwd("..")  # knitr wd is where the .Rmd lives. Go up one level
  setwd()
} else {
  # code executed interactively:
  top_dir <- getwd()  # top directory given by RStudio project
}
...
data_file <- file.path(results_dir, "all_data.csv")
raw_dat <- read.table(data_file, header=TRUE, sep=",")  # no dropped trials.

# load pre-processed data (used for fitting)
load(file.path(results_dir, "final_data.RData"))

# load model fit (see expt_11_fit_sum_code_model_stan.R):
load(file.path(results_dir, "model_fit.RData"))

# load helper functions:
source(file.path(top_dir, "code", "analysis", "helper_funs.R"))

# save data and fit to a named variable to avoid mistaking for subsequent experiments:
dat_1 <- dat
raw_dat_1 <- raw_dat
fit_1 <- fit
fit_additive_1 <- fit_additive
loo_1 <- loo
loo_additive_1 <- loo_additive

rm(raw_dat, dat, fit, fit_additive, loo, loo_additive)
@
...
<<cap-num-params, echo=FALSE, cache=TRUE>>=
cap <- "Approximate number of parameters matched by each texture model assessed in the present paper. The dashed line shows the number of pixels in a 256-pixel-square image. Models above the line are overcomplete."
@

<<num-params, echo=FALSE, cache=TRUE, fig.width=fig_width_1_col, fig.height=3, fig.cap=cap, fig.env="figure">>=

# create data:
num_params <- data.frame(model = c("conv1", "conv2", "conv3", "conv4", "conv5", "PS", "powerspec"),
                         n_params = c(2000, 10000, 43000, 175000, 306000, 1000, 209408))

num_params$thousand_params <- num_params$n_params / 1000

plt <- ggplot(num_params, aes(x = reorder(model, thousand_params), y = thousand_params)) + 
  geom_col() + 
  geom_hline(yintercept = (256*256) / 1000, linetype = "dashed") + 
  xlab("") + ylab("Number of parameters (thousands)")

plt <- plot_theme(plt) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
plt
...
<<experiment_1a_data_preprocessing, echo=FALSE>>=
dat_1a <- dat_1 %>% 
  filter(subj == "TW" | subj == "CF")

n_trials_each <- dat_1a %>% group_by(subj) %>% summarise(n = n())

if (n_trials_each$n[1] != n_trials_each$n[2]){
  warning("Observers have done different numbers of trials!")
}
...
The authors completed five experiment sessions (each consisting of one parafoveal and one inspection block), for a total of \Sexpr{n_trials_each$n[1]} trials each.
The order of presentation conditions was pseudorandomly determined for each author in each experiment session.
The dataset consisted of \Sexpr{nrow(dat_1a)} trials.

<<experiment_1b_data_preprocessing, echo=FALSE>>=
raw_1b <- raw_dat_1 %>% 
  filter(subj != "TW" & subj != "CF")
...
missing_trials <- raw_1b %>%
  group_by(subj, correct) %>%
  summarise(n = n()) %>%
  mutate(prop = n / sum(n)) %>%
...
The second group (Experiment 1b) consisted of ten\footnote{Ten was chosen \textit{a priori} based on pilot testing.} na\"{i}ve observers (median age \Sexpr{median(dat_1b$age)} years, min \Sexpr{min(dat_1b$age)}, max \Sexpr{max(dat_1b$age)}), who completed only one experimental session each (i.e. one block of each presentation time)\footnote{
...
The final dataset consisted of \Sexpr{nrow(dat_1b)} trials.

\subsection{Results}

<<plot_settings, echo=FALSE>>=
line_alpha <- 0.4
ribbon_alpha <- 0.2
point_size <- 0.3  # for model estimates
nudge_amount <- 0  # position nudge for point offsets.
@

<<cap_expt-1-summary, echo=FALSE>>=
subj_means_1a <- dat_1a %>%
  group_by(subj, image_model, presentation_cond) %>%
  summarise(pc = mean(correct), n = n())

subj_means_1b <- dat_1b %>%
  group_by(subj, subj_display, image_model, presentation_cond) %>%
  summarise(pc = mean(correct), n = n())

cap <- paste0("Performance as a function of image model in Experiment 1, averaging over images. 
              For the authors (CF and TW), points show the mean proportion correct and error bars show 95\\% bootstrapped confidence intervals. Each data point represents ",
              median(subj_means_1a$n), " trials. 
...
<<expt-1-summary, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=2.5, fig.cap=cap, fig.env="figure*", warning=FALSE>>=
plt <- ggplot(dat_1a, aes(x = image_model, y = correct,
                          shape = presentation_cond,
                          colour = presentation_cond)) + 
  geom_hline(yintercept = 1/3, linetype = "dashed") + 
  facet_wrap(~ subj_display, ncol = 3)  # subject display facets this into CF, TW and Naive.


# compute model predictions for individual subjects:
subj_predictions <- expand.grid(subj = c("TW", "CF"),
                                image_model = levels(dat_1a$image_model),
                                presentation_cond = levels(dat_1a$presentation_cond))
subj_predictions$image_code <- NA
blah <- fitted(fit_1, newdata = subj_predictions, scale = "response",
               re_formula = ~ (image_model * presentation_cond | subj), 
               allow_new_levels = TRUE)

subj_predictions <- cbind(subj_predictions, blah)
subj_predictions$subj_display <- subj_predictions$subj

# plot model predictions for TW and CF:
plt <- plt + 
  geom_ribbon(data = subj_predictions,
              aes(ymax = `97.5%ile`, ymin = `2.5%ile`, y = NULL,
                  group = presentation_cond,
                  fill = presentation_cond, colour = NULL), alpha = ribbon_alpha) +
  geom_line(data = subj_predictions, aes(y = Estimate, group = presentation_cond)) 

# this plots the individual observer data (TW and CF, because default dataframe is 1a):
plt <- plt + stat_summary(fun.data = "mean_cl_boot",
                          position = position_dodge(width=nudge_amount),
                          size = point_size)

# compute model predictions for population:
model_dat <- marginal_effects(fit_1, effects = "image_model:presentation_cond")
model_dat <- model_dat$`image_model:presentation_cond`
model_dat$subj_display <- factor("Naive", levels = levels(dat_1a$subj_display))

# plot subject level marginals:
plt <- plt + 
  geom_ribbon(data = model_dat,
              aes(ymax = `upper__`, ymin = `lower__`, y = NULL,
                  group = presentation_cond,
                  fill = presentation_cond, colour = NULL), alpha = ribbon_alpha) +
  geom_line(data = model_dat,
            aes(y = estimate__, group = presentation_cond))

plt <- plt + stat_summary(data = subj_means_1b,
                          aes(y = pc),
...
# add subject means:
for (i in 1 : length(unique(subj_means_1b$subj))){
  this_subj <- unique(subj_means_1b$subj)[i]
  this_dat <- subj_means_1b %>% filter(subj == this_subj)

  # this gets too cluttered, unfortunately.
  # plt <- plt + geom_line(data = this_dat,
  #                        aes(y = pc, group = presentation_cond),
  #                        alpha = line_alpha)
...
plt <- plt + coord_cartesian(ylim = c(0, 1)) +
  scale_y_continuous(name = "Proportion correct", breaks = c(0, .5, 1)) +  
  xlab("Image model") + ylab("Proportion correct")

# fix scales:
plt <- plt + scale_colour_fivethirtyeight(name = "")
plt <- plt + scale_fill_fivethirtyeight(name = "")
plt <- plt + scale_shape(name = "")
# plt <- plot_theme(plt) + theme(legend.position = c(0.5, 0.15),
#                                legend.spacing = unit(1, units = "pt"),
#                                legend.direction = "horizontal")
plt <- plot_theme(plt) + theme(legend.position = "top")
...
% individual images.
<<cap_expt-1-images, echo=FALSE>>=

subj_means_1a <- dat_1a %>%
  group_by(subj, image_model, presentation_cond, image_name) %>%
  summarise(pc = mean(correct), n = n())
...
cap <- paste0("Performance for each image in Experiment 1. 
              Points show the grand mean of all observer means (based on ", median(subj_means_1a$n)," trials for authors and ", median(subj_means_1b$n)," trials for naives). 
              ...
              The original images can be seen in Figure \\ref{fig:original_stimuli}.")
@

<<expt-1-images, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=4.5, fig.cap=cap, fig.env="figure*", warning=FALSE>>=

# reorder panels of images to put consistent results together:
dat_1$image_name <- factor(dat_1$image_name, c(
  "Beans",
  "Bricks",  
  "Flowers",
  "Grass",
...
plt <- ggplot(dat_1, aes(x = image_model, y = correct,
                         shape = presentation_cond,
                         colour = presentation_cond)) + 
  geom_hline(yintercept = 1/3, linetype = "dashed") + 
  facet_wrap(~ image_name) 

## model predictions at image level:
predictions <- expand.grid(image_model = levels(dat_1a$image_model),
                           presentation_cond = levels(dat_1a$presentation_cond),
                           image_code = levels(dat_1a$image_code))
predictions$subj <- NA
blah <- fitted(fit_1, newdata = predictions,
               scale = "response",
               re_formula = ~ (image_model * presentation_cond | image_code), 
               allow_new_levels = TRUE)


predictions <- cbind(predictions, blah)
predictions$subj_display <- predictions$subj

# need to include image code for plotting:
predictions$image_name <- predictions$image_code
predictions$image_name <- revalue(predictions$image_name, 
...
plt <- plt +
  geom_ribbon(data = predictions,
              aes(ymax = `97.5%ile`, ymin = `2.5%ile`, y = NULL,
                  group = presentation_cond,
                  fill = presentation_cond, colour = NULL), alpha = ribbon_alpha) + 
  geom_line(data = predictions, aes(y = Estimate, group = presentation_cond))


## subject data averages:
subj_data <- dat_1 %>% 
  group_by(subj, image_name, image_model, presentation_cond) %>% 
  summarise(pc = mean(correct), n = n())

# plot with mean and SE:
plt <- plt + stat_summary(data = subj_data, aes(y = pc),
                          fun.data = "mean_se", 
                          # position = position_dodge(width=nudge_amount),
                          size = point_size)
...
<<load_experiment_2_data, echo=FALSE, results='hide'>>=
...
# load model fit (see expt_12_fit_sum_code_model_stan.R):
load(file.path(results_dir, "model_fit.RData"))
...
n_trials_each_expt_2 <- dat_2 %>% group_by(subj) %>% summarise(n = n())
s1 <- n_trials_each_expt_2 %>% filter(subj == "S1")

rm(raw_dat, dat, fit, loo)
...
All observers completed two experiment sessions (each consisting of one parafoveal and one inspection block) for a total of \Sexpr{n_trials_each_expt_2$n[1]} trials,  with the exception of S1, who did not return for a second session of testing and so completed only \Sexpr{s1$n} trials.


\subsection{Results}

<<cap_expt-2-summary, echo=FALSE>>=

subj_means <- dat_2 %>%
  group_by(subj, image_model, presentation_cond) %>%
  summarise(pc = mean(correct), n = n())

cap <- paste0("Performance as a function of image model in Experiment 2, averaging over images.
              Points show mean and 95\\% confidence intervals on performance (each based on ", 
              median(subj_means$n), " trials for all observers except S2).
              Lines show mixed-effects model predictions for each observer (mean of posterior regression line) and shaded regions show model 95\\% credible intervals, ignoring random effects of image. ")
@

<<expt-2-summary, echo=FALSE, cache=TRUE, fig.width=fig_width_1_col, fig.height=3.5, fig.cap=cap, warning=FALSE>>=
plt <- ggplot(dat_2, aes(x = image_model, y = correct,
                         shape = presentation_cond,
                         colour = presentation_cond)) + 
  geom_hline(yintercept = 1/3, linetype = "dashed") + 
...
# # compute model predictions for individual subjects:
subj_predictions <- expand.grid(subj = unique(dat_2$subj),
                                image_model = levels(dat_2$image_model),
                                presentation_cond = levels(dat_2$presentation_cond))
blah <- fitted(fit_2, newdata = subj_predictions, scale = "response",
               re_formula = ~ (image_model * presentation_cond | subj))
...
plt <- plt + scale_fill_fivethirtyeight(name = "")
plt <- plt + scale_shape(name = "")
plt <- plot_theme(plt) + theme(legend.position = c(0.75, 0.15),
                               legend.spacing = unit(1, units = "pt"))
plt
@

<<cap_expt-2-images, echo=FALSE>>=
subj_data <- dat_2 %>% 
  group_by(subj, image_name, image_model, presentation_cond) %>% 
  summarise(pc = mean(correct), n = n())


cap <- paste0("Performance for each image in Experiment 2.
              Points show means and $\\pm 1 SEM$ over observer means.
              Faint lines link individual observer mean performance (based on ",
              median(subj_data$n),
              " trials for all observers except S2).
              Solid lines show mixed-effects model estimates (posterior mean) and shaded regions show 95\\% credible intervals.")
@

<<expt-2-images, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=4.5, fig.cap=cap, fig.env="figure*", warning=FALSE>>=

# reorder panels of images to put consistent results together:
dat_2$image_name <- factor(dat_2$image_name, c(
  "Beans",
  "Bricks",  
  "Flowers",
  "Grass",
...
## model predictions at image level:
predictions <- expand.grid(image_model = levels(dat_2$image_model),
                           presentation_cond = levels(dat_2$presentation_cond),
                           image_code = levels(dat_2$image_code))
blah <- fitted(fit_2, newdata = predictions,
               scale = "response",
               re_formula = ~ (image_model * presentation_cond | image_code))
...
for (i in 1 : length(unique(subj_data$subj))){
  this_subj <- unique(subj_data$subj)[i]
  this_dat <- subj_data %>% filter(subj == this_subj)
  # ls <- "solid"
  
  plt <- plt + geom_line(data = this_dat, aes(y = pc, group = presentation_cond), 
                         alpha = 0.4)
}
...
<<cap_cross_corr, echo=FALSE>>=
cap <- paste0("Controlling for texture model overfit. 
              ...
              Some images are more self-similar, and thus require some degree of copying to match appearance.")
@


<<cross_corr, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=3.5, fig.cap=cap, fig.env="figure*", warning=FALSE>>=
# import data, stick scales together:

process_one_file <- function(dat){
  # re-order data using order from Alex's code:
  idx <- c(12, 1, 4, 6, 11, 2, 3, 7, 9, 5, 8, 10)
  dat <- dat[idx, ]
  ...
  dat$name <- NULL
  dat$PS <- dat$ps_synths
  dat$ps_synths <- NULL
  return(dat)
}

cross_cor_dat <- data.frame()
scales <- c(32, 64, 128)

for (i in 1 : length(scales)){
  blah <- read.csv(file = file.path(top_dir, "results", "copy_machine_crosscor", paste0("data", scales[i], ".csv")))
  blah <- process_one_file(blah)
  blah$scale <- scales[i]
  cross_cor_dat <- rbind(cross_cor_dat, blah)
}


# reshape
cross_cor_models <- cross_cor_dat %>% 
  gather(model, xcor, conv5, powerspec, PS, original) %>% 
  filter(model != "original")

cross_cor_orig <- cross_cor_dat %>% 
  gather(model, xcor, conv5, powerspec, PS, original) %>% 
  filter(model == "original")

# plot
plt <- ggplot(cross_cor_models, aes(x = reorder(im_code, xcor), y = xcor, colour = model, fill = model)) + 
  facet_wrap(~ scale, ncol = 3) + 
  geom_col(data = cross_cor_orig, aes(fill = NULL, colour = NULL), alpha = 0.2) +
  # geom_crossbar(data = cross_cor_orig, aes(ymin = xcor, ymax = xcor, fill = NULL, colour = NULL), alpha = 0.5) +
  geom_point() + 
  scale_colour_fivethirtyeight(name = "") + 
  scale_fill_fivethirtyeight(name = "") + 
  scale_y_continuous(limits = c(0, 1), breaks = c(0, 0.5, 1)) + 
  labs(x = "", y = "Maximum cross-correlation")
...
<<mcmc_methods, echo=FALSE, cache=TRUE>>=
options(scipen = 20)

stan_pars <- fit_1$fit@stan_args
n_chains <- length(stan_pars)
algorithm <- stan_pars[[1]]$algorithm
thin <- stan_pars[[1]]$thin
warmup <- stan_pars[[1]]$warmup
samples <- stan_pars[[1]]$iter
final_samples <- (samples - warmup) / thin * n_chains
@
...
Consider that the model coefficients are defined on the linear predictor scale, whose effective range runs from approximately -5 (returning an expected success probability of \Sexpr{round(plogis(-5), 3)}) to 5 (returning \Sexpr{round(plogis(5), 3)}; a linear predictor value of zero gives \Sexpr{round(plogis(0), 3)}).
...
Samples were drawn using the \Sexpr{algorithm} sampling algorithm \citep{hoffman_nouturn_2014} with \Sexpr{n_chains} independent chains, each sampled with \Sexpr{samples} samples of which \Sexpr{warmup} were used to adaptively tune the sampler (warmup).
To reduce the final file size we saved every \Sexpr{thin}th sample.
This procedure resulted in a final total of \Sexpr{final_samples} post-warmup samples.
...
<<model_linpred_estimates, echo=FALSE, cache=TRUE>>=

fe_samples <- posterior_samples(fit_1, pars = "^b")  # extract fixed effect samples.

# the pres cond with sum coding is half the linear predictor difference between conditions (i.e. double to get full difference -- see expt_11_test_contrast_coding.Rmd). With inspection coded as -1, parafoveal as 1, a positive value gives parafoveal > inspection. Flip sign by multiplying by -2:
pres_cond_ests <- compute_CIs(fe_samples$b_presentation_cond1 * -2)

printout_1 = paste0("$\\exp(", round(pres_cond_ests$estimates[2], 2), ") = ",
       round(exp(pres_cond_ests$estimates[2]), 2), "$")

# with presentation_cond coded as sum, the coefficients for image_model
# code the mean differences between levels. To get the marginal "simple effects"
# differences for each level of presentation cond, need to add or subtract the
# relevant interaction term. See rmd doc "expt_11_test_contrast_coding.Rmd"
# showing this with simulated data.

# parafoveal (coded 1)
single_fix_image_2m1 <- fe_samples$b_image_model2M1 + fe_samples$`b_image_model2M1:presentation_cond1`
single_fix_image_3m2 <- fe_samples$b_image_model3M2  + fe_samples$`b_image_model3M2:presentation_cond1`
single_fix_image_4m3 <- fe_samples$b_image_model4M3 + fe_samples$`b_image_model4M3:presentation_cond1`
single_fix_image_5m4 <- fe_samples$b_image_model5M4 + fe_samples$`b_image_model5M4:presentation_cond1`
single_fix_image_6m5 <- fe_samples$b_image_model6M5 + fe_samples$`b_image_model6M5:presentation_cond1`

# inspection:
inspection_image_2m1 <- fe_samples$b_image_model2M1 - fe_samples$`b_image_model2M1:presentation_cond1`
inspection_image_3m2 <- fe_samples$b_image_model3M2 - fe_samples$`b_image_model3M2:presentation_cond1`
inspection_image_4m3 <- fe_samples$b_image_model4M3 - fe_samples$`b_image_model4M3:presentation_cond1`
inspection_image_5m4 <- fe_samples$b_image_model5M4 - fe_samples$`b_image_model5M4:presentation_cond1`
inspection_image_6m5 <- fe_samples$b_image_model6M5 - fe_samples$`b_image_model6M5:presentation_cond1`


# Effect size estimates -----------------------------
# need to add the variance components that contribute to the mean difference. That is, images 
# and subjects.
var_samples <- posterior_samples(fit_1, pars = "^sd")
# convert all vals from sd to var:
var_samples <- as.data.frame(var_samples^2)

# EF = mean difference / sqrt(var_subj + var_images)

# pres cond EF:
pres_cond_ef <- fe_samples$b_presentation_cond1 / 
  sqrt(var_samples$sd_subj__presentation_cond1 + 
         var_samples$sd_image_code__presentation_cond1)


## OLD, when I used treatment coding. Needs to be updated for sum!
# # parafoveal
# single_fix_image_2m1_ef <- fe_samples$b_image_model2M1 / 
#   sqrt(var_samples$sd_subj__image_model2M1 + 
#          var_samples$sd_image_code__image_model2M1)
# single_fix_image_3m2_ef <- fe_samples$b_image_model3M2 / 
#   sqrt(var_samples$sd_subj__image_model3M2 + 
#          var_samples$sd_image_code__image_model3M2)
# single_fix_image_4m3_ef <- fe_samples$b_image_model4M3 / 
#   sqrt(var_samples$sd_subj__image_model4M3 + 
#          var_samples$sd_image_code__image_model4M3)
# single_fix_image_5m4_ef <- fe_samples$b_image_model5M4 / 
#   sqrt(var_samples$sd_subj__image_model5M4 + 
#          var_samples$sd_image_code__image_model5M4)
# single_fix_image_6m5_ef <- fe_samples$b_image_model6M5 / 
#   sqrt(var_samples$sd_subj__image_model6M5 + 
#          var_samples$sd_image_code__image_model6M5)
# 
# # inspection:
# inspection_image_2m1_ef <- (fe_samples$b_image_model2M1 + 
#                               fe_samples$`b_image_model2M1:presentation_cond2`) /
#   sqrt(var_samples$sd_subj__image_model2M1 + 
#          var_samples$`sd_subj__image_model2M1:presentation_cond2` + 
#          var_samples$sd_image_code__image_model2M1 + 
#          var_samples$`sd_image_code__image_model2M1:presentation_cond2`)
# 
# inspection_image_3m2_ef <- (fe_samples$b_image_model3M2 + 
#                               fe_samples$`b_image_model3M2:presentation_cond2`) /
#   sqrt(var_samples$sd_subj__image_model3M2 + 
#          var_samples$`sd_subj__image_model3M2:presentation_cond2` + 
#          var_samples$sd_image_code__image_model3M2 + 
#          var_samples$`sd_image_code__image_model3M2:presentation_cond2`)
# inspection_image_4m3_ef <- (fe_samples$b_image_model4M3 + 
#                               fe_samples$`b_image_model4M3:presentation_cond2`) /
#   sqrt(var_samples$sd_subj__image_model4M3 + 
#          var_samples$`sd_subj__image_model4M3:presentation_cond2` + 
#          var_samples$sd_image_code__image_model4M3 + 
#          var_samples$`sd_image_code__image_model4M3:presentation_cond2`)
# inspection_image_5m4_ef <- (fe_samples$b_image_model5M4 + 
#                               fe_samples$`b_image_model5M4:presentation_cond2`) /
#   sqrt(var_samples$sd_subj__image_model5M4 + 
#          var_samples$`sd_subj__image_model5M4:presentation_cond2` + 
#          var_samples$sd_image_code__image_model5M4 + 
#          var_samples$`sd_image_code__image_model5M4:presentation_cond2`)
# inspection_image_6m5_ef <- (fe_samples$b_image_model6M5 + 
#                               fe_samples$`b_image_model6M5:presentation_cond2`) /
#   sqrt(var_samples$sd_subj__image_model6M5 + 
#          var_samples$`sd_subj__image_model6M5:presentation_cond2` + 
#          var_samples$sd_image_code__image_model6M5 + 
#          var_samples$`sd_image_code__image_model6M5:presentation_cond2`)

## EDIT: decided not to include these effect sizes because I'm not sure about the covariance components (see Westfall et al, 2014, Appendix). So, I'll leave it for now.
@
...
The posterior median of the difference between these conditions on the linear predictor scale is \Sexpr{round(compute_CIs(fe_samples$b_presentation_cond1 * -2)$estimates[2], 2)}. % multiply by -2 to get full difference, with inspection above.
Considering the exponent of this value as log odds, this means that correct trials are \Sexpr{printout_1} times more likely under the inspection condition than the parafoveal condition, if all other effects are held at zero.
...
\Sexpr{round(compute_CIs(fe_samples$b_presentation_cond1 * -2)$estimates[1], 2)} and
\Sexpr{round(compute_CIs(fe_samples$b_presentation_cond1 * -2)$estimates[3], 2)}.
...
\Sexpr{format(compute_CIs(fe_samples$b_presentation_cond1 * -2)$p_less_zero, scientific = TRUE)}).
...
%\Sexpr{round(hypothesis(fit_1, "presentation_cond2 = 0")$hypothesis$Evid.Ratio, 2)}, indicating that the posterior density at zero is much smaller than the prior density (offering evidence against the null).
...
(\Sexpr{print_CIs(fe_samples$b_presentation_cond1 * -2)}).
...
\Sexpr{round(2 * compare(loo_additive_1, loo_1)[1], 0)} (SE = \Sexpr{round(2 * compare(loo_additive_1, loo_1)[2], 0)})).
...
(\Sexpr{print_CIs(single_fix_image_2m1, fit_1, "image_model2M1 = 0")}),
and conv3 was worse than conv2
(\Sexpr{print_CIs(single_fix_image_3m2, fit_1, "image_model3M2 = 0")}).
However, because performance was now approximately at chance, there was no evidence that conv4 was different to conv3
(\Sexpr{print_CIs(single_fix_image_4m3, fit_1, "image_model4M3 = 0")})
or that conv5 was different to conv4
(\Sexpr{print_CIs(single_fix_image_5m4, fit_1, "image_model5M4 = 0")}).
Similarly, the PS model was also not different to conv5
(\Sexpr{print_CIs(single_fix_image_6m5, fit_1, "image_model6M5 = 0")}).
...
(\Sexpr{print_CIs(inspection_image_2m1, fit_1, "image_model2M1 + image_model2M1:presentation_cond1 = 0")}) and
conv3 produced worse performance than conv2
(\Sexpr{print_CIs(inspection_image_3m2, fit_1, "image_model3M2 + image_model3M2:presentation_cond1 = 0")}).
Conv4 produced worse performance than conv3 in that the coefficient coding their difference was likely to be negative
(\Sexpr{print_CIs(inspection_image_4m3, fit_1, "image_model4M3 + image_model4M3:presentation_cond1 = 0")}).
Performance for the conv5 model was approximately equal to conv4
(\Sexpr{print_CIs(inspection_image_5m4, fit_1, "image_model5M4 + image_model5M4:presentation_cond1 = 0")}).
...
(\Sexpr{print_CIs(inspection_image_6m5, fit_1, "image_model6M5 + image_model6M5:presentation_cond1 = 0")}).
...
<<compute_re_ratios, echo=FALSE, cache=TRUE>>=
re_samples <- posterior_samples(fit_1, pars = "^sd")  # extract random effects SDs.
half <- ncol(re_samples) / 2
ratios <- re_samples[, 1:half] / re_samples[, (half+1):ncol(re_samples)]  # image code SDs / subj SDs

estimates <- sapply(ratios, median)  # median for each sd estimate

# Variance associated with dependence of PS and conv5 diff under presentation condition:
ratio_6M5 <- re_samples$`sd_image_code__image_model6M5` / re_samples$`sd_subj__image_model6M5`
@
...
On average, the variance associated with images is \Sexpr{round(median(estimates), 1)} times greater than that associated with observers.
The linear predictor difference between PS and conv5 averaged over presentation condition is associated with about \Sexpr{round(median(ratio_6M5), 1)} times more variance from images than from observers.
...
<<cap_prob-sensitive-1, echo=FALSE>>=
cap <- paste0("Posterior probability that performance for discriminating each image and image model in Experiment 1 lies above chance.
              ...
              Conditions for which the model predicts exactly chance performance would fall at 0.5.") 
@


<<prob-sensitive-1, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=4.5, fig.cap=cap, fig.env="figure*">>=

predictions <- expand.grid(image_model = levels(dat_1a$image_model),
                           presentation_cond = levels(dat_1a$presentation_cond),
                           image_code = levels(dat_1a$image_code))

# return posterior samples for all conditions. Each col is for one row of predictions.
blah <- fitted(fit_1, newdata = predictions,
               scale = "response",
               re_formula = ~ (image_model * presentation_cond | image_code),
               summary = FALSE)
...
compute_P <- function(x){
  # use ecdf to compute the probability that the coefficient is greater than 1/3:
  fn <- ecdf(x)
  
  # fn computes mass lying below the given value. i.e. fn(0) = 0 and fn(1) = 1.
  # we want the mass lying above 1/3.
  # i.e. 1 - fn(1/3)
  return(1 - fn(1/3))  
}

p <- apply(blah, 2, compute_P)
predictions$p <- p
...
# reorder panels of images to put consistent results together:
predictions$image_name <- factor(predictions$image_name, c(
  "Beans",
  "Bricks",  
  "Flowers",
  "Grass",
...
plt <- ggplot(predictions, aes(x = image_model, y = p,
                               shape = presentation_cond,
                               colour = presentation_cond)) + 
  geom_hline(yintercept = 0.95, linetype = "dashed") +
  geom_hline(yintercept = 0.05, linetype = "dashed") +
  facet_wrap(~ image_name) + 
  geom_point() + geom_line(aes(group = presentation_cond))

plt <- plt + 
  coord_cartesian(ylim = c(0, 1)) +
  scale_y_continuous(name = "Posterior probability of performance > chance", breaks = c(0, .5, 1)) +
  xlab("Image model")
...
<<model_linpred_estimates_2, echo=FALSE, cache=TRUE>>=
fe_samples_2 <- posterior_samples(fit_2, pars = "^b")  # extract fixed effect samples.
...
# parafoveal (coded 1)
single_fix_image_2m1_2 <- fe_samples_2$b_image_model2M1 + fe_samples_2$`b_image_model2M1:presentation_cond1`
single_fix_image_3m2_2 <- fe_samples_2$b_image_model3M2 + fe_samples_2$`b_image_model3M2:presentation_cond1` 

# inspection (coded -1)
inspection_image_2m1_2 <- fe_samples_2$b_image_model2M1 - fe_samples_2$`b_image_model2M1:presentation_cond1`
inspection_image_3m2_2 <- fe_samples_2$b_image_model3M2 - fe_samples_2$`b_image_model3M2:presentation_cond1` 
@
...
(\Sexpr{print_CIs(single_fix_image_2m1_2, fit_2, "image_model2M1 = 0")}), 
whereas under inspection the PS model was easier to discriminate from the original images than the conv5 model
(\Sexpr{print_CIs(inspection_image_2m1_2, fit_2, "image_model2M1 + image_model2M1:presentation_cond2 = 0")}).
...
(\Sexpr{print_CIs(single_fix_image_3m2_2, fit_2, "image_model3M2 = 0")}),
but better performance than the PS model under inspection
(\Sexpr{print_CIs(inspection_image_3m2_2, fit_2, "image_model3M2 + image_model3M2:presentation_cond2 = 0")}).
...
<<cap_prob-sensitive-2, echo=FALSE>>=
cap <- paste0("Posterior probability that performance for each image in Experiment 2 lies above chance.
              Plot elements as in Figure \\ref{fig:prob-sensitive-1}.") 
@


<<prob-sensitive-2, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=4.5, fig.cap=cap, fig.env="figure*">>=
...
<<cap-oddball-performance-average, echo=FALSE, cache=TRUE>>=
cap <- "Performance in Experiment 1 according to whether the oddball image was an original or a model synthesis (``synth''), averaging over
images. Points show grand mean across observer means, error bars show SEM."
@

<<oddball-performance-average, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=3.5, fig.cap=cap, fig.env="figure*", warning=FALSE>>=
av_dat <- dat_1 %>%
  group_by(subj, image_model, oddball, presentation_cond) %>% 
  summarise(pc = mean(correct))

plt <- ggplot(av_dat, aes(x = image_model, y = pc,
                         shape = oddball,
                         colour = oddball)) + 
  geom_hline(yintercept = 1/3, linetype = "dashed") + 
  facet_wrap(~ presentation_cond, ncol = 2)
...
<<cap-oddball-performance-images-parafoveal, echo=FALSE, cache=TRUE>>=
cap <- "Parafoveal performance in Experiment 1 according to whether the oddball image was an original or a model synthesis (``synth''), for each image.
        Points show grand mean across observer means, error bars show SEM."
@

<<oddball-performance-images-parafoveal, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=4.5, fig.cap=cap, fig.env="figure*", warning=FALSE>>=
av_dat <- dat_1 %>%
  filter(presentation_cond == "Parafoveal") %>% 
  group_by(subj, image_model, oddball, image_name) %>% 
  summarise(pc = mean(correct))
...
<<cap-oddball-performance-images-inspection, echo=FALSE, cache=TRUE>>=
cap <- "Inspection performance in Experiment 1 according to whether the oddball image was an original or a model synthesis (``synth''), for each image.
        Points show grand mean across observer means, error bars show SEM."
@

<<oddball-performance-images-inspection, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=4.5, fig.cap=cap, fig.env="figure*", warning=FALSE>>=
av_dat <- dat_1 %>%
  filter(presentation_cond == "Inspection") %>% 
  group_by(subj, image_model, oddball, image_name) %>% 
  summarise(pc = mean(correct))
...
<<load-loss-data, cache=TRUE, echo=FALSE, warning=FALSE>>=
experiment_num <- 11

# here we load the main data file: 
results_dir <- file.path(top_dir, "results", paste0("experiment_", experiment_num))
loss_iter <- read.csv(file.path(results_dir, "loss_iterations_10versions.csv"))
loss_iter$version <- factor(loss_iter$version)

final_loss <- loss_iter %>% 
  group_by(model, version) %>% 
  mutate(final_loss = loss[iteration == max(iteration)]) %>% 
  summarise(final_loss = mean(final_loss)) %>% 
  left_join(y = num_params, by = "model")

final_loss$model <- factor(final_loss$model)

final_loss_av <- final_loss %>% 
  group_by(model) %>% 
  summarise(final_loss = mean(final_loss))  %>% 
  left_join(y = num_params, by = "model")

final_loss_av$model <- factor(final_loss_av$model)
@
...
The final loss after 1000 iterations is superlinear (Figure \ref{fig:gradient-descent}B): for example, conv5 has a little less than double the number of parmameters as conv4, but about \Sexpr{round(final_loss_av$final_loss[final_loss_av$model == "conv5"] / final_loss_av$final_loss[final_loss_av$model == "conv4"], 0)} times higher final loss.
...
<<cap-gradient-descent, echo=FALSE>>=
cap <- paste0("\\textbf{a}: Decrease of the loss over iterations on a logarithmic scale for ten syntheses (lines) of one example image (Bricks). Loss for simple models (e.g. conv1) converges to zero whereas for more complex models (conv3, conv4 and conv5) it stabilises in a local minimum.
              ...
              Final loss is superlinear in the number of parameters. ")
@

<<gradient-descent, echo=FALSE, cache=TRUE, fig.width=fig_width_2_col, fig.height=3, fig.cap=cap, fig.env="figure*", warning=FALSE>>=

# examples of gradient descent:
plt1 <- ggplot(loss_iter, aes(x = iteration, y = loss, 
                              colour = model)) + 
  geom_line(aes(group = version:model), alpha = 0.5) +
  scale_y_log10() +
  scale_x_continuous(breaks = seq(0, 1000, by = 250)) + 
  # scale_x_log10(name = "Iteration", breaks = c(1, 10, 100, 1000)) +
  scale_colour_brewer(name = "", type = "qual", palette = 2)
plt1 <- plot_theme(plt1) + 
  labs(x = "Iteration", y = "Loss", title = "a") + 
...
# final loss against number of params:
plt2 <- ggplot(final_loss, aes(x = thousand_params, y = final_loss), 
               colour = model) + 
  geom_point(aes(group = version:model, colour = model), alpha = 0.5) + 
  geom_line(data = final_loss_av) +
  scale_y_log10(breaks = c(1, 10, 100, 1000, 5000)) +
  # scale_x_log10(name = "Iteration", breaks = c(1, 10, 100, 1000)) +
  scale_colour_brewer(name = "", type = "qual", palette = 2)

plt2 <- plot_theme(plt2) + 
  labs(x = "Number of parameters (thousands)", y = "Final loss", title = "b") + 
  theme(plot.title = element_text(size = 12, face = "bold")) + 
