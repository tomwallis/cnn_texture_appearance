...
%However, in this paper we will consider the appearance and matching of natural photographic textures rather than the statistics of artificially-generated dot patterns.
...
% [TODO: probably remove this paragraph] \citet{balas_attentive_2008} had observers sort texture images into groups of similar-looking textures, then examined the ability of three feature spaces (power spectrum, \citet{heeger_pyramidbased_1995} and \citet{portilla_parametric_2000}) to predict the perceptual clusters (posed as a multiclass classification problem).
% The results showed that all models could classify human decisions better than chance and that the power spectrum model outperformed the other two.
% While this indicates that the variance in similarity in this task is mostly related to power-spectral similarity, the relative failure of the two texture synthesis spaces may also reflect a paucity of training data, since the power spectrum alone should be predictable from those feature spaces.
...
% The pooling operation means that in higher layers the size of the feature maps decreases, whereas the number of feature maps increases. 
% Conversely, these smaller feature maps correspond to a larger region of the original image (a loose analogy can be made to the increasing size of receptive fields in visual neurons in ascending the ventral hierarchy).
...
%The raw dataset for this group consisted of nrow(raw_1b) trials; of these, observers failed to respond in nrow(raw_dat[is.na(raw_1b$correct), ]) trials.
...
% \subsection{Individual observer data}
...
% The performance of individual na\"{i}ve subjects from Experiment 1 is shown in Figure \ref{fig:expt-1b-images}.
% The poor constraint at the individual subject level is evident in the plot: each data point for each subject represents only five trials.
% This limits the amount we can infer about individual subjects, but their data help to constrain the population-level estimates.
% 
% In contrast, authors CF and TW are measured comparatively precisely. 
% We visualise the model predictions for one of these observers (TW) for all images below (Figure \ref{fig:expt-1-tw}.
% Note that the model uncertainty is much smaller compared to that in Figure \ref{fig:expt-1-summary}, because of the heterogeneity of the results for individual images.
% The model nevertheless provides quite reasonable fits to data at the individual (subject, image) level.
% This highlights one advantage of using mixed models for psychophysical data \citep{cheung_nonlinear_2008,knoblauch_modeling_2012,moscatelli_modeling_2012}: multiple sources of variability can be accounted for and incorporated into predictions at various levels (e.g. the subject + image level, or the subject level ignoring images, or the population level).
